...
void set_button_interrupts(void) {
    // cowpi_register_pin_ISR(1 << 3, flip_leds);
    static mbed::InterruptIn right_button((PinName)3);
    right_button.mode(PullUp);
    right_button.rise(flip_leds);
    right_button.fall(flip_leds);
}

void set_ticker(void) {
    static mbed::Ticker ticker;
    ticker.attach(flip_leds, std::chrono::seconds(1));
}
